...
const suName = i_name.toUpperCase();
const suReg = i_registration.toUpperCase();
const suDriver = i_driver.toUpperCase();
const sqlFind =
	"SELECT id FROM fleet.vehicle WHERE $1 = ANY (fims_names) " +
	"AND $2 = ANY (fims_registrations) " +
	"AND $3 = ANY (fims_drivers) ";

const aFind = plv8.execute(sqlFind, [suName, suReg, suDriver]);
if (aFind.length == 1) {
	...
	const aFindReg = plv8.execute(sqlFindReg, [suReg]);
