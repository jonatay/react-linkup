...
function* loadAllAttendLogs({ payload }) {
  ...
  yield put(
    attendLogActions.loadAttendLogsFulfilled(attendLogs)
  );
}
function* createAttendLog({ payload: { attendLog } }) {
  ...
  yield put(
    attendLogActions.createAttendLogFulfilled(
      result.attendLog
    )
  );
}
...
function* updateAttendLog({ payload: { id, changes } }) {
  let result = yield call(
    [attendLogList, attendLogList.update],
    id,
    { changes }
  );
  yield put(
    attendLogActions.updateAttendLogFulfilled(
      result.attendLog
    )
  );
}
...
function* removeAttendLog({ payload: { attendLog } }) {
  let result = yield call(
    [attendLogList, attendLogList.remove],
    attendLog.id
  );
  if (result.status === 'deleted') {
    yield put(
      attendLogActions.removeAttendLogFulfilled(attendLog)
    );
  } else {
...
function* watchLoadAttendLogs() {
  yield takeEvery(
    attendLogActions.LOAD_ATTEND_LOGS,
    loadAllAttendLogs
  );
}
...
function* watchCreateAttendLog() {
  yield takeEvery(
    attendLogActions.CREATE_ATTEND_LOG,
    createAttendLog
  );
}
...
function* watchUpdateAttendLog() {
  yield takeEvery(
    attendLogActions.UPDATE_ATTEND_LOG,
    updateAttendLog
  );
}
...
function* watchRemoveAttendLog() {
  yield takeEvery(
    attendLogActions.REMOVE_ATTEND_LOG,
    removeAttendLog
  );
}
